...
import { AISummarizer, getAIConfig } from './ai.js';
// Mock Issue data
const mockIssue = {
    number: 123,
...
describe('AISummarizer', () => {
    ...
    describe('summarizeIssue', () => {
        it('should successfully summarize an issue', async () => {
            ...
            expect(fetch).toHaveBeenCalledWith(mockAIConfig.url, expect.objectContaining({
                method: 'POST',
        ...
        it('should handle API error', async () => {
            ...
            const summarizer = new AISummarizer(mockAIConfig);
            await expect(summarizer.summarizeIssue(mockIssue)).rejects.toThrow('Failed to summarize issue: AI API error: undefined - Rate limit exceeded');
        });
        it('should handle network error', async () => {
            global.fetch = vi.fn().mockRejectedValue(new Error('Network error'));
            const summarizer = new AISummarizer(mockAIConfig);
            await expect(summarizer.summarizeIssue(mockIssue)).rejects.toThrow('Failed to summarize issue: Network error');
        });
