...
import { generateEmailTemplate, type IssueSummary } from './lib/email-template.js';
...
async function processIssues(): Promise<void> {
  ...
  if (issues.length === 0) {
    ...
    await emailService.sendEmail(
      `[Issue Picket] ${repoOwner}/${repoName} - 今日无优先级Issue`,
      `<!DOCTYPE html>
<html><body style="font-family: sans-serif; padding: 20px;">
  ...
  await emailService.sendEmail(
    `[Issue Picket] ${repoOwner}/${repoName} - 发现 ${issueSummaries.length} 个优先级Issue`,
    htmlContent
  );
